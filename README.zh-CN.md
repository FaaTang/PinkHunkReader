...
- **文件夹工作区**：打开目录、树形浏览、多标签；支持 Ctrl/Cmd 多选、Shift 连续选中与鼠标左键拖拽框选；右键可对选中项批量定位或移出工作区（移出后若文件夹在工作区内已无剩余项则一并移除，根为空则移出该根）；刷新会重新枚举已打开根下的全部文件（含此前主动移出的项）；可隐藏/显示资源管理器（默认 `Ctrl+,` / `Cmd+,`）；可拖拽资源管理器右边缘调节宽度（会记住）
- **新建文件**：`Ctrl+N` / `Cmd+N`，关闭未保存时提示保存；首次另存后只把该文件加入资源管理器，不打开其所在文件夹
...
- **PDF**：按页预览（PDF.js）
- **Word**：`.docx` 只读预览（HTML）；有标题时可显示大纲。旧版 `.doc` 不预览，请另存为 `.docx` 后再打开
...
- **源文件缺失**：已打开的文件被删除或重命名后，可编辑的文本 / Markdown（非分页）会询问是否在编辑器内保留缓冲（保存时按原路径重建）；分页与二进制类型仅提示源文件已失效。保留后的 orphan 标签不会自动保存，需手动 Save。
- **路径沙箱**：读写限制在已打开根目录内
- **最近打开**：Open Recent 下列出最近打开的文件与文件夹；数量可在 Settings → General 中调整（默认 10）
...
| `Ctrl+Shift+F` / macOS `Cmd+Shift+F` | 打开文件 |
| `Ctrl+Shift+D` / macOS `Cmd+Shift+D` | 打开文件夹 |
