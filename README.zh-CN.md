...
- **文件夹工作区**：打开目录、树形浏览、多标签；支持 Ctrl/Cmd 多选、Shift 连续选中与鼠标左键拖拽框选；右键可对选中项批量定位或移出工作区（移出后若文件夹在工作区内已无剩余项则一并移除，根为空则移出该根）；刷新会重新枚举已打开根下的全部文件（含此前主动移出的项）；可隐藏/显示资源管理器（默认 `Ctrl+,` / `Cmd+,`）
